...
class GenderClassifier:
    ...
    def loadData(self, filename1, filename2, modelfile_prefix):
        ...
        print col
        print data
        self.X = csr_matrix((data, (row, col)),
                            shape=(len(genders), self.num_fea))
        self.X = self.X[inds, :]
        self.Y = self.Y[inds]
        print self.Y
        print len(self.Y)
        ...
        t1 = time.time()
        scores = cross_validation.cross_val_score(self.clf, self.X, self.Y,
                                                  cv=self.cv)
        t2 = time.time()
...
def main(params_file):
    ...
    # params['process_country_gender_output_file_gender_zh'], params['gender_classifier_model_file_zh'])
...
if __name__ == '__main__':
    ...
    assert os.path.isfile(
        params_file), 'params-file %s is not a valid file!' % params_file
    main(params_file)
